...
const AdminListings = () => {
  ...
  const toggleActive = async (company) => {
    try {
      if (company.isActive) {
        if (!window.confirm(`Deactivate "${company.name}"? It will be hidden from the public catalog.`)) return
        await api.patch(`/companies/${company._id}/deactivate`)
        setFlash(`Deactivated "${company.name}"`)
      } else {
